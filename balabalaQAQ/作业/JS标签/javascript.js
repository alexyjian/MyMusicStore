...
window.onload = function () {
    ...
    var current_index = 0;
    //启动定时器
    var timer = setInterval(autoCheck, 3000);
    ...
    for (var i = 0; i < head_divs.length; i++) {
        ...
        head_divs[i].onmouseover = function () {
            ...
            if (i != current_index) {
                ...
                head_divs[current_index].style.borderBottom = '';44444444444444444444444444444444444
            }
            //获取所有的tab-body-ul
            var body_uls = document.getElementById('tab-body').getElementsByTagName('ul');
            //遍历所有的tab-body-ul
            for (var i = 0; i < body_uls.length; i++) {
                //将所有的元素隐藏 利用替换去掉current类名
        ...
        head_divs[i].onmouseout = function () {
            timer = setInterval(autoCheck, 3000);
        }
    }
    //定义定时器，按周期检查tab栏的切换
    function autoCheck() {
